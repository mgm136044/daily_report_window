...
```
04:05  launchd (macOS) · 작업 스케줄러 (Windows)   [run] schedule_time
...
논리적 하루는 **04:00 ~ 다음날 04:00** 이다 (`[day] boundary_hour`). 새벽 2시 작업은
전날로 잡힌다. 실행 시각은 `[run] schedule_time` 이고 기본값이 04:05 — 하루가 완전히
닫힌 직후다.

경계 이후면 아무 때나 되고 보고 대상 날짜도 같다. **늦은 쪽이 나은 경우가 많다.**
새벽 4시엔 PC 가 꺼져 있으면 그냥 안 돌지만, 09:00 은 자리에 앉는 시각에 잡히고
여전히 어제를 보고한다. 경계보다 이른 값은 설치 시점에 거부한다 — 그 시각엔 보고할
하루가 아직 안 닫혔기 때문이다.

**하루가 끝나기 전에 그날 것을 보려면** 창의 `오늘 지금까지` 버튼을 쓴다.
...
누르면 상태 창이 열린다 — 마지막 실행, 다음 실행, 최근 14일 스트립, 그리고 버튼 줄.
`pythonw` 로 뜨므로 뒤에 빈 콘솔이 따라오지 않는다.

| 버튼 | |
|---|---|
| 설정… | 요약 엔진(claude/codex), Codex 모델, 예약 시각 |
| 진단 실행 | `doctor` 를 아래 출력창에 |
| *날짜* 다시 생성 | 마감된 가장 최근 하루를 다시 만든다 |
| 오늘 지금까지 | **하루가 끝나기 전의 오늘** — 아래 참고 |
| 다른 날짜… | 날짜를 직접 입력 |
| 예약 작업 등록 | 예약 작업을 다시 등록 |
| 로그 열기 / Notion 열기 | |

**`오늘 지금까지`** 는 퇴근할 때 쓰라고 있다. 지금까지 된 것으로 만들고, 그건 하루가
아니라 스냅숏이라 **일부러 장부에 기록하지 않는다.** 그래야 그 날짜가 밀린 목록에
남아서, 하루가 닫힌 뒤 예약 실행이 완전한 판을 만들어 스냅숏을 덮어쓴다. 언제든 다시
눌러 갱신해도 된다.

패널은 명령이 끝날 때마다 장부를 다시 읽는다 — 창을 닫았다 열 필요가 없다.
**`예약 작업 등록`** 이 있는 이유는, 업그레이드가 예약 작업을 지우는데 예전엔 설정
마법사만 그걸 되돌려 놓았기 때문이다. 이제는 창이 알아서 눈치채고 다시 등록하며,
버튼은 그게 실패했을 때를 위한 것이다.
...
파일을 열지 않아도 된다 — 상태 창의 **`설정…`** 버튼이 엔진·모델·예약 시각을
바꾼다. 쓰기 전에 TOML 로 파싱해 보고, `config.toml.bak` 을 남기고, 주석은 전부
제자리에 둔다.

설치기는 claude 와 codex 를 둘 다 찾아본다. **둘 다 있으면 claude 를 쓴다** —
이 프로젝트가 시간을 가장 많이 쓴 경로라서다. codex 만 있으면 그쪽으로 설정한다.
