...
윈도우에는 창이 둘 더 있다. 둘 다 순수 tkinter다 — 의존성을 들이지 않는다.
...
마지막 실행, 다음 실행, 최근 14일 스트립, 그리고 위 명령들을 부르는 버튼.
`install.ps1` 이 시작 메뉴 바로 가기를 만든다.
...
- 원본 프롬프트와 명령은 **기기 밖으로 나가지 않는다.** 게시되는 건 모델이 쓴 산문뿐이다.
